...
export default function RoadmapTracker({ params }: { params: { id: string } }) {
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-12 animate-pulse bg-background min-h-screen">
        <div className="bg-surface border border-surfaceBorder p-8 mb-8 shadow-sm">
          <div className="flex gap-3 mb-4">
            <div className="h-6 w-24 bg-surfaceBorder rounded-sm"></div>
            <div className="h-6 w-32 bg-surfaceBorder rounded-sm"></div>
          </div>
          <div className="h-10 w-3/4 bg-surfaceBorder mb-4 rounded-sm"></div>
          <div className="h-6 w-1/2 bg-surfaceBorder mb-6 rounded-sm"></div>
          <div className="h-8 w-64 bg-surfaceBorder/50 rounded-sm"></div>
        </div>
        <div className="mb-10">
          <div className="flex justify-between items-end mb-2">
            <div className="h-5 w-32 bg-surfaceBorder rounded-sm"></div>
            <div className="h-4 w-48 bg-surfaceBorder rounded-sm"></div>
          </div>
          <div className="w-full bg-surfaceBorder h-2 rounded-full"></div>
        </div>
        <div className="space-y-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex gap-6">
              <div className="w-8 h-8 bg-surfaceBorder rounded-full shrink-0"></div>
              <div className="flex-1 h-32 bg-surface/50 border border-surfaceBorder rounded-sm"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background transition-colors duration-300">
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="bg-surface border border-surfaceBorder p-8 mb-8 shadow-sm transition-colors duration-300">
          ...
          <h1 className="font-heading text-4xl font-bold mb-2 uppercase text-foreground">{params.id.replace(/-/g, " ")}</h1>
          <p className="text-lg text-foreground/70 flex items-center gap-2 mb-6">
        ...
        <div className="mb-10">
          <div className="flex justify-between items-end mb-2">
            ...
            <p className="text-sm text-foreground/60 font-medium">You have completed {completedCount} of {stages.length} stages</p>
          </div>
          <div className="w-full bg-surfaceBorder h-2 rounded-full overflow-hidden border border-surfaceBorder">
                    ...
                    <div className="filter blur-[4px] opacity-40 pointer-events-none select-none">
                      <div className="flex gap-6">
                        <div className="flex flex-col items-center">
                           <Circle className="w-8 h-8 text-foreground/20 bg-background rounded-full" />
                           <div className="w-0.5 h-48 -mt-2 bg-surfaceBorder"></div>
                        </div>
                        <div className="flex-1 p-6 border bg-surface/50 border-surfaceBorder rounded-sm mb-4">
                           <h3 className="font-heading text-2xl font-bold text-foreground/50 mb-2">Stage {stage.id}: {stage.title}</h3>
                           <p className="text-foreground/60 mb-4">{stage.desc}</p>
                           <div className="h-10 bg-surfaceBorder w-1/3 rounded-sm mb-2"></div>
                           <div className="h-10 bg-surfaceBorder w-1/2 rounded-sm"></div>
                        </div>
                      </div>
                    </div>
              ...
              <div key={stage.id} className="relative flex gap-6">
                <div className="flex flex-col items-center">
                  <div className="bg-background z-10 py-1">
                    {isCompleted && <CheckCircle className="w-8 h-8 text-success bg-background rounded-full shadow-sm" />}
                    {isActive && <Circle className="w-8 h-8 text-primary fill-primary/20 bg-background rounded-full shadow-sm" />}
                    {isLocked && <Circle className="w-8 h-8 text-foreground/20 bg-background rounded-full" />}
                  </div>
                  {index !== stages.length - 1 && (
                    <div className={`w-0.5 h-full -mt-2 transition-colors duration-500 ${isCompleted ? "bg-success" : "bg-surfaceBorder"}`}></div>
                  )}
                </div>

                <div className={`flex-1 p-6 border mb-4 transition-all duration-300 ${
                  isCompleted ? "bg-surface border-success/30 shadow-sm" : 
                  isActive ? "bg-surface border-primary shadow-md transform scale-[1.01]" : 
                  "bg-surface/50 border-surfaceBorder opacity-60"
                }`}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className={`font-heading text-2xl font-bold ${isLocked ? "text-foreground/50" : "text-foreground"}`}>
                      Stage {stage.id}: {stage.title}
                    </h3>
                    <span className={`text-xs font-bold px-3 py-1 uppercase tracking-wider rounded-sm border ${
                      isCompleted ? "bg-success/10 text-success border-success/20" : 
                      isActive ? "bg-primary/10 text-primary border-primary/20" : "bg-foreground/5 text-foreground/50 border-surfaceBorder"
                    }`}>
                      {stage.status.replace('-', ' ')}
                    </span>
                  </div>
                  
                  <p className="text-foreground/80 mb-6">{stage.desc}</p>

                  {(isActive || isCompleted) && stage.checklist && (
                    <div className="mb-6">
                      <p className="font-bold text-sm text-foreground mb-3 uppercase tracking-wider">Checklist</p>
                          ...
                          return (
                            <li 
                              ...
                              className={`flex items-start gap-3 p-3 rounded-sm transition-colors border ${
                                isLocked ? "cursor-not-allowed border-transparent" : "cursor-pointer hover:bg-foreground/5 border-transparent hover:border-surfaceBorder"
                              }`}
                            >
                              {isItemChecked ? (
                                <CheckSquare className="w-5 h-5 text-success shrink-0 mt-0.5" />
                              ) : (
                                <Square className={`w-5 h-5 shrink-0 mt-0.5 ${isLocked ? "text-foreground/20" : "text-foreground/40"}`} />
                              )}
                    ...
                    <div className="bg-warning/10 border-l-4 border-warning p-4 mb-6 flex gap-3">
                      ...
                      <p className="text-sm text-foreground/90 font-medium">{stage.notes}</p>
                    </div>
                  )}
                  ...
                  {isActive && isPremium && (
                    <button 
                      ...
                      className="bg-primary text-white font-bold py-3 px-8 hover:bg-primaryHover transition-colors mt-2 shadow-sm flex items-center gap-2 rounded-sm"
                    >
