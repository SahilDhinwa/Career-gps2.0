import { CheckCircle, Circle, MapPin, Calendar, BookOpen, ChevronRight } from "lucide-react";
...
export default function RoadmapTracker() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      {/* Header Info */}
      <div className="bg-surface border border-surfaceBorder p-8 mb-8">
      ...
      <div className="mb-10">
        <div className="flex justify-between items-end mb-2">
          ...
          <p className="text-sm text-gray-500">You have completed {completedCount} of {stages.length} stages</p>
        </div>
        <div className="w-full bg-gray-200 h-2">
          <div className="bg-success h-2 transition-all duration-500" style={{ width: `${progressPercent}%` }}></div>
        </div>
      </div>
            ...
            <div key={stage.id} className="relative flex gap-6">
              ...
              <div className="flex flex-col items-center">
                <div className="bg-background z-10 py-1">
                  {isCompleted && <CheckCircle className="w-8 h-8 text-success bg-white rounded-full" />}
                  {isActive && <Circle className="w-8 h-8 text-primary fill-primary/20 bg-white rounded-full" />}
                  {isLocked && <Circle className="w-8 h-8 text-gray-300 bg-white rounded-full" />}
                </div>
                {index !== stages.length - 1 && (
                  <div className={`w-0.5 h-full -mt-2 ${isCompleted ? "bg-success" : "bg-gray-200"}`}></div>
                )}
              </div>

              {/* Stage Card */}
              <div className={`flex-1 p-6 border mb-4 transition-all ${
                isCompleted ? "bg-surface border-success/30" : 
                isActive ? "bg-white border-primary shadow-md" : 
                ...
                {isActive && (
                  <button className="bg-primary text-white font-bold py-2 px-6 hover:bg-primaryHover transition-colors mt-2">
                    Mark as Complete
                  </button>
                )}
              </div>
