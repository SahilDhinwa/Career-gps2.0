...
import { ArrowLeft, CheckCircle2, XCircle, RefreshCcw, Beaker } from "lucide-react";

const DRAVYAGUNA_PAPER_1_TEST_1 = [
...
export default function DravyagunaPaper1Test1() {
  ...
  const handleAnswerClick = (index: number) => {
    ...
    setIsAnswered(true);
    if (index === DRAVYAGUNA_PAPER_1_TEST_1[currentQuestion].correctAnswerIndex) {
      setScore(score + 1);
    }
  };
  ...
  const handleNextQuestion = () => {
    ...
    if (nextQuestion < DRAVYAGUNA_PAPER_1_TEST_1.length) {
        ...
        {showScore ? (
          <div className="bg-surface/80 backdrop-blur-md border border-surfaceBorder rounded-sm p-10 text-center shadow-xl">
            ...
            <p className="text-xl text-foreground/80 font-medium mb-8">
              You scored <span className="text-amber-500 font-bold text-3xl">{score}</span> out of {DRAVYAGUNA_PAPER_1_TEST_1.length}
            </p>
          ...
          <div className="bg-surface/80 backdrop-blur-md border border-surfaceBorder rounded-sm p-8 md:p-10 shadow-xl">
            <div className="flex justify-between items-center mb-8 border-b border-surfaceBorder pb-4">
              <span className="text-amber-500 font-bold text-sm tracking-wider uppercase flex items-center gap-2">
                <Beaker className="w-4 h-4" /> Paper 1 (Test 1)
              </span>
              <span className="text-foreground/50 font-bold text-sm">
                Question {currentQuestion + 1} / {DRAVYAGUNA_PAPER_1_TEST_1.length}
              </span>
            </div>
            
            <h2 className="text-xl md:text-2xl font-bold mb-8 leading-relaxed">
              {DRAVYAGUNA_PAPER_1_TEST_1[currentQuestion].questionText}
            </h2>
            
            <div className="space-y-4">
              {DRAVYAGUNA_PAPER_1_TEST_1[currentQuestion].options.map((option, index) => {
            ...
            {isAnswered && (
              <div className="mt-8 flex justify-end">
                <button onClick={handleNextQuestion} className="bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-8 rounded-sm transition-colors shadow-sm">
                  {currentQuestion + 1 === DRAVYAGUNA_PAPER_1_TEST_1.length ? "Finish Test" : "Next Question"}
                </button>
              </div>
            )}
          </div>
